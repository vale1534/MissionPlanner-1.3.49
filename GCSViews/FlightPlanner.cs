...
namespace MissionPlanner.GCSViews
{
    public partial class FlightPlanner : MyUserControl, IDeactivate, IActivate
    {
        ...
        //FIM - VICTOR
        //VICTOR - tava private mudei pra public e era void
        public void Savewaypoints()
        {
            using (SaveFileDialog fd = new SaveFileDialog())
            {
                ...
                //INICIO - VICTOR - escrevi primeira linha e comenta a segunda que ja era do codigo
               // this.file = fd.FileName;
                ...
                if (file != "")
                {
                    try
                    {
                        ...
                        for (int a = 0; a < Commands.Rows.Count - 0; a++)
                        {
                            ...
                            sw.Write((a + 1)); // seq
                            sw.Write("\t" + 0); // current ERA UM 0 SÓ
...
Column 1: Field type (RALLY is the only one at the moment -- may have RALLY_LAND in the future)
 ...
 Column 7: Flags (just 0 for now, also future use).
        ...
        //INICIO - VICTOR - Botão de envio do arquivo de waypoints para o coordenador
        private void btnsendcoord_Click(object sender, EventArgs e)
        {
            Con_Coord formcoord = new Con_Coord();
            Coord_CON coordenador = new Coord_CON();

            //string fileName = "tt.txt";// "Your File Name";
            //string filePath = @"C:\txt\";//Your File Path;

            SaveFileDialog fd = new SaveFileDialog();
            string fileNName = fd.FileName;
            //string filePPath = @Path.GetFullPath(fd.FileName);
            MessageBox.Show(fileNName.ToString());
            // MessageBox.Show(filePPath);
            // coordenador.IP = formcoord.IpAddress;
            // coordenador.PORT = Convert.ToInt32(formcoord.Port);

            //coordenador.Send_File(fileNName, filePPath);
